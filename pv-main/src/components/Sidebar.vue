<template>
  <aside 
    class="fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col transform transition-transform duration-300 lg:transform-none shrink-0"
...
const navItems = computed(() => [
  ...
  {
    name: 'thermalDetection',
    path: `/${locale.value}/thermal-detection`,
    label: 'nav.thermalDetection',
    icon: Thermometer
  },
  {
    name: 'maintenance',
...
<style scoped>
.nav-item {
  @apply flex items-center space-x-3 px-4 py-3 rounded-lg text-text-light dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200 cursor-pointer select-none;
  position: relative;
  z-index: 10;
}
