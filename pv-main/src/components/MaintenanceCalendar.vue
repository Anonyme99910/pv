  ...
  <div>
    ...
    <div class="grid grid-cols-7 gap-1 sm:gap-2">
      ...
      <div
        ...
        :key="day"
        class="text-center text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400 py-1 sm:py-2"
      >
        {{ day.substring(0, 1) }}<span class="hidden sm:inline">{{ day.substring(1) }}</span>
      </div>
      ...
      <div
        ...
        :key="day.date"
        class="aspect-square p-1 sm:p-2 rounded-lg border transition-colors cursor-pointer"
        :class="dayClass(day)"
        @click="selectDay(day)"
      >
        <div class="text-xs sm:text-sm font-medium mb-0.5 sm:mb-1">{{ day.day }}</div>
        <div v-if="day.taskCount > 0" class="flex items-center justify-center">
          <span class="text-[10px] sm:text-xs px-1 sm:px-2 py-0.5 bg-primary text-white rounded-full">
            {{ day.taskCount }}
          </span>
        </div>
      </div>
